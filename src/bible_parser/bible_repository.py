...
class BibleRepository:
    ...
    def _populate_database(self) -> None:
        ...
        if self.xml_string is not None:
            parser = BibleParser.from_string(
                self.xml_string, format=self.format)
        ...
        try:
            for book in parser.books:
                # Insert book
                cursor.execute(
                    "INSERT OR IGNORE INTO books (id, num, title, long_title, short_title, abbreviation) VALUES (?, ?, ?, ?, ?, ?)",
                    (book.id, book.num, book.title, book.long_title,
                     book.short_title, book.abbreviation),
                )
    ...
    def get_books(self) -> List[Book]:
        ...
        cursor = self._db.cursor()
        cursor.execute(
            "SELECT id, num, title, long_title, short_title, abbreviation FROM books ORDER BY num")
    ...
    def _migrate_schema(self) -> None:
        ...
        if self._db is None:
            return
        cursor = self._db.cursor()
        for column in ("long_title", "short_title", "abbreviation"):
            try:
